...
if (!isset($_SESSION['ads'])) {
    ...
    // Вывод объявлений, если они есть и вывод объявления в форму для редактирования
    $ads = $_SESSION['ads'];
    $ad  = isset($_SESSION['ads'][$number]) ? $_SESSION['ads'][$number] : '';
}
    ...
    if (function_exists($function_name)) {
        ...
        if (!empty($_POST)) {
            ...
            if (!empty($number) or ($number === 0)) {
                ...
                $host = 'http://'.$_SERVER['HTTP_HOST'].'/'.$routes_array[1].'/ads/index/id/'.$number;

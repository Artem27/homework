...
function display_ads ($post, $validate_post) {
...
$display_ads = display_ads($_POST ,$display_error);
...
if ( !empty($_POST) && !empty($display_error) ) {
    $ads_edit    = $_POST;
...
if ( isset($_GET['edit']) ) {
    ...
    /* {Добавляем переменные смарти} */
    $smarty->assign ('ads_edit', $_SESSION['ads'][$_GET['edit']]);
    $smarty->assign ('checked', $checked);
    $smarty->assign ('city', $city);
    $smarty->assign ('metro', $metro);
    $smarty->assign ('category_id', $category_id);
...
/* {Сортируем массив с объявлениями после удаления} */
if ( isset($_SESSION['ads']) ) {
    sort($_SESSION['ads']);
    $smarty->assign ('ads', $_SESSION['ads']);
}
